...
function CalendarPicker({visible, initialStart, initialEnd, allBars, onApply, onClose}: CalendarProps) {
  ...
  const [calMonth, setCalMonth] = useState(endM - 1);
  ...
  function handleDayPress(iso: string) {
    ...
    if (tempStart && tempEnd) {
      ...
      if (iso > tempStart) { setTempEnd(iso); }
      else { setTempEnd(tempStart); setTempStart(iso); }
    } else {
      setTempStart(iso);
    }
  }
  ...
  const hint = tempStart && tempEnd ? 'Ketuk hari untuk memilih ulang'
    : tempStart ? 'Pilih tanggal akhir' : 'Pilih tanggal awal';
  const canApply = !!(tempStart && tempEnd);

  function getDayStyle(iso: string) {
    return {
      isStart:  iso === tempStart,
      isEnd:    iso === tempEnd,
      inRange:  !!(tempStart && tempEnd && iso > tempStart && iso < tempEnd),
      isToday:  iso === todayISO,
      disabled: iso < minDate || iso > maxDate,
      hasData:  dataDates.has(iso),
    };
  }
      ...
      <View style={cal.overlay}>
        <View style={cal.card}>
          ...
          <View style={cal.monthNav}>
            <TouchableOpacity onPress={prevMonth} disabled={!canPrev} style={cal.navBtn} hitSlop={{top:8,bottom:8,left:8,right:8}}>
              <Text style={[cal.navArrow, !canPrev && cal.navArrowDisabled]}>‹</Text>
            </TouchableOpacity>
            <Text style={cal.monthLabel}>{MONTHS_ID[calMonth]} {calYear}</Text>
            <TouchableOpacity onPress={nextMonth} disabled={!canNext} style={cal.navBtn} hitSlop={{top:8,bottom:8,left:8,right:8}}>
              <Text style={[cal.navArrow, !canNext && cal.navArrowDisabled]}>›</Text>
            </TouchableOpacity>
          </View>
          <View style={cal.weekRow}>
            {DAYS_SHORT.map(d => <Text key={d} style={cal.weekDay}>{d}</Text>)}
          </View>
          {rows.map((row, ri) => (
            <View key={ri} style={cal.weekRow}>
              {row.map((day, ci) => {
                ...
                return (
                  <TouchableOpacity
                    key={ci}
                    style={[cal.dayCell, inRange && cal.dayCellInRange, isSelected && cal.dayCellSelected]}
...
function buildChart(vals: number[], chartW: number) {
  ...
  const vPad   = ((rawMax - rawMin) * 0.25) || 1;
  ...
  const areaD = n > 1 ? `${lineStr} L${pts[n - 1].x},${bottomY} L${pts[0].x},${bottomY} Z` : '';
...
export function LivinTrenChart({data}: Props) {
  ...
  const skipDotVal = filteredBars.length > 14;
  ...
  return (
    <View style={styles.card}>
      <Text style={styles.title}>Tren UREG per Hari</Text>

      <TouchableOpacity style={styles.dateBtn} onPress={() => setShowCal(true)} activeOpacity={0.8}>
        ...
        {chart ? (
          <Svg width={chartW} height={CHART_H}>
            {chart.gridVals.map((gv, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  <Line x1={LEFT_PAD} y1={y} x2={chartW - RIGHT_PAD} y2={y}
                    stroke={Colors.border} strokeWidth={0.8} strokeDasharray="4,3" />
                  <SvgText x={LEFT_PAD - 4} y={y + 3.5} fontSize={8} fill={Colors.textSecondary} textAnchor="end">
                    {gv}
                  </SvgText>
                </React.Fragment>
              );
            })}
            {chart.areaD ? <Path d={chart.areaD} fill={Colors.primary} fillOpacity={0.10} /> : null}
            {vals.length > 1 && (
              <Polyline points={chart.ptsStr} fill="none" stroke={Colors.primary}
                strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            )}
            {chart.dots.map((dot, i) => (
              <Circle key={i} cx={dot.x} cy={dot.y} r={3.5}
                fill={Colors.primary} stroke={Colors.surfaceLight} strokeWidth={1.5} />
            ))}
            {!skipDotVal && chart.dots.map((dot, i) => (
              <SvgText key={i} x={dot.x} y={dot.y - 7} fontSize={9} fontWeight="700"
            ...
            {filteredBars.map((bar, i) => {
              ...
              return (
                <SvgText key={i} x={chart.xPos(i)} y={CHART_H - 4}
